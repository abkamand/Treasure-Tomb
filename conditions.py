...
def diamond_from_dead_mummy(player):
    ...
    """
    if (
        player.current_location.name == "ash cluster room 1"
    ):  # check to see if the player is in the right room
        for (
            enemies
        ) in (
            ...
            if (
                enemies.name == "mummy" and enemies.is_dead == True
            ):  # name must match and must be dead
                print(
                    'As the mummy vanishes into dust, he screeches, "You will not make it out of this tomb alive!" As his body disintegrates, he reaches into his rotting chest and pulls out a diamond from it.'
                )
                ...
                diamond.add_description(
                    "A red diamond that fell from the body of a mummy after you defeated it."
                )
